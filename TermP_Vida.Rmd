---
title: "Length-Weight Relationship of *Sarotherodon melanotheron* In Dominli Lagoon - Ghana"
author: "Vida S. Osei"
date: "5/3/2017"
output:
...
##2.2. Fish Data 
![Fig 1. *S. melanotheron*](https://nas.er.usgs.gov/XIMAGESERVERX/2005/20051102151738.jpg "*S. melanotheron*")
...
labs(title="Fig2. Length Distribution",x="Standard Length(cm)", y = "Counts") # adding labels to plot
```
...
This plots shows body of fish species according to sex. The lowest median weight was observed in April for both sexes and heighest in June for both sexes also.However, the overall median weight was higher in females than males.
...
ggplot(data, aes(x=Month, y=BW.g)) + 
  ...
  facet_grid(Sex ~ .)+ #split plot to double panels
  labs(title="Fig3. Plot of Body Weight by Month",x="Month", y = "Weight")+
  theme_classic()
```


### 3.3 Gonad Weight
Median weight of Ovaries in Females was highest in the month of February (fig 4). Weight of ovaries was least in the months of September, October, May and April.
...
ggplot(data_F, aes(x=Month, y = GW.g )) + 
 geom_boxplot(fill = 'green')+ # Plot of Gonad weight by Month
  labs(title=" Fig4. Gonad Weight of Females",x="Month", y = "Weight")+
  theme_classic()
```
...
geom_point()+
  ggtitle(" Fig 5. Length Weigtht Relationship of Blackchin Tilapia")+
